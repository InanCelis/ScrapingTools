...
class IdealHomeInternational {
    private string $baseUrl = "https://idealhomesinternational.com";
    private string $foldername = "IdealHomeInternational";
    ...
    private function extractPropertyLinks(simple_html_dom $html): void {
        ...
        foreach ($html->find('.listing-view.grid-view h2.item-title a') as $a) {
            $href = $a->href ?? '';
    ...
    private function scrapePropertyDetails(simple_html_dom $html, $url): void {
        ...
        if ($detailWrap) {
            ...
            if (preg_match('/<strong>Price:<\/strong>\s*<span>([^<]+)<\/span>/', $detailsHtml, $matches)) {
                ...
                if (preg_match('/[\d,.\s]+/', $priceText, $priceMatches)) {
                    $cleanPrice = $priceMatches[0];
                    ...
                    if (strpos($cleanPrice, '.') !== false) {
                        $cleanPrice = substr($cleanPrice, 0, strpos($cleanPrice, '.'));
                    }
            ...
            $allowed_type = ['Villa', 'Condo', 'Apartment', 'House', 'Penthouse', 'Casa', 'Studio','Studios', 'Home', 'Hotel'];
        ...
        $addressWrap = $html->find('#property-address-wrap .list-2-cols li');

        $city = '';
        $state = '';
        $country = '';

        // Extract values from the list items
        foreach ($addressWrap as $item) {
            if (strpos($item->class, 'detail-city') !== false) {
                $city = trim($item->find('span', 0)->plaintext);
            } elseif (strpos($item->class, 'detail-state') !== false) {
                $state = trim($item->find('span', 0)->plaintext);
            } elseif (strpos($item->class, 'detail-country') !== false) {
                $country = trim($item->find('span', 0)->plaintext);
            }
        }
